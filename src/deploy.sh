...
case "$TYPE" in
  frontend_cloudfront_s3)
    /deploy_scripts/deploy_frontend_cf.sh --project="$PROJECT" --app="$APP" --env="$ENVIRONMENT"
    ;;
  frontend_ecs_fargate)
    /deploy_scripts/deploy_frontend_ecs.sh --project="$PROJECT" --app="$APP" --env="$ENVIRONMENT"
    ;;
  webservice_sls_python)
    /deploy_scripts/deploy_sls.sh --project="$PROJECT" --app="$APP" --env="$ENVIRONMENT"
    ;;
  webservice_ecs_fargate)
    /deploy_scripts/deploy_ecs.sh --project="$PROJECT" --app="$APP" --env="$ENVIRONMENT"
    ;;
  *)
